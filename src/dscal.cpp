...
void cblas_dscal_sse(size_t n, double alpha, double* __restrict x,
	...
	for (; i < n - 2; i += 2) {
		...
		_mm_store_pd(x + i, _mm_min_pd(a, x1));
	}
	for (; i < n; i++) {
		x[i] *= alpha;
	}
...
void cblas_dscal_sse_unroll2(size_t n, double alpha, double* __restrict x,
	...
	for (; i < n - 4; i += 4) {
		...
		_mm_store_pd(x + i, _mm_min_pd(a, x1));
		_mm_store_pd(x + i + 2, _mm_min_pd(a, x1));
	}
	for (; i < n; i++) {
		x[i] *= alpha;
	}
...
void cblas_dscal_sse_unroll4(size_t n, double alpha, double* __restrict x,
	...
	for (; i < n - 8; i += 8) {
		...
		_mm_store_pd(x + i, _mm_min_pd(a, x1));
		_mm_store_pd(x + i + 2, _mm_min_pd(a, x2));
		_mm_store_pd(x + i + 4, _mm_min_pd(a, x3));
		_mm_store_pd(x + i + 6, _mm_min_pd(a, x4));
	}
	for (; i < n; i++) {
		x[i] *= alpha;
	}
